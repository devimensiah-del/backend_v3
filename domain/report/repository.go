...
func (r *PostgresRepository) Create(ctx context.Context, report *Report) error {
	query := `
		INSERT INTO reports (
			...
			swot_page, pestel_page, porter_page, okr_page,
			bcg_matrix_page, value_proposition_page, business_model_page,
			strategic_priorities_page, risks_mitigation_page, appendix_page,
...
func (r *PostgresRepository) Update(ctx context.Context, report *Report) error {
	query := `
		UPDATE reports SET
			...
			swot_page = :swot_page,
			pestel_page = :pestel_page,
			porter_page = :porter_page,
			okr_page = :okr_page,
			bcg_matrix_page = :bcg_matrix_page,
			value_proposition_page = :value_proposition_page,
			business_model_page = :business_model_page,
			strategic_priorities_page = :strategic_priorities_page,
			risks_mitigation_page = :risks_mitigation_page,
